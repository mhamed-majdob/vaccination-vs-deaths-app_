...
def load_data():
    ...
    deaths = pd.read_csv(os.path.join(base, 'data', 'estimated-cumulative-excess-deaths-per-100000-people-during-covid-19.csv'))
...
with ov1:
    st.markdown('<div class="card"><div class="card-h">💉 Vaccination Rollout</div><div class="card-p">Daily doses per million people (7-day average). Higher = faster rollout.</div>', unsafe_allow_html=True)
    fig_v = go.Figure()
    for c in selected:
...
with ov2:
    st.markdown('<div class="card"><div class="card-h">💀 Excess Deaths</div><div class="card-p">Cumulative excess deaths per 100,000 people. Higher = more deaths above normal levels.</div>', unsafe_allow_html=True)
    fig_d = go.Figure()
    for c in selected:
        cd = merged_data[merged_data['Entity'] == c]
        col = COLORS.get(c, '#8B949E')
...
for country in selected:
    ...
    with ch1:
        ...
        st.markdown(f'<div class="card"><div class="card-h">{country} — Vaccination vs Excess Deaths</div><div class="card-p">Each dot = one date. The line shows the overall trend.</div>', unsafe_allow_html=True)
        fig_s = go.Figure()
